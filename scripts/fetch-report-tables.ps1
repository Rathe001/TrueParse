...
E1 "-- tanks took it; share = fraction of all damage taken. Feeds"
E1 "-- DeathCause: names why a player died (avoidable / tankbuster / chip)."
E1 ("-- Generated {0} - {1}." -f $today, $zone.name)
E1 "local _, TP = ..."
E1 ""
...
foreach ($encName in ($dmg.Keys | Sort-Object)) {
    ...
    foreach ($nm in ($D.ab.Keys | Sort-Object)) {
        ...
        E1 ("E[`"{0}`"][`"{1}`"] = {{ hitRate = {2}, tankOnly = {3}, share = {4}, guid = {5}, n = {6} }}" -f `
            $enckey, ($nm -replace '"', '\"'), $hitRate, ($(if ($tankOnly) { "true" } else { "false" })), $share, $A.guid, $D.playerN)
    }
